...
2026-08-23 用户进一步报告：修改前每天多次后处理卡住 10–20 秒，当前 hedging 版本在一天正常使用中零次复现。已在官方 Ideas 发布 [Discussion #1953](https://github.com/cjpais/Handy/discussions/1953)，征求默认关闭 opt-in 方案的社区反馈。在维护者/社区决定 `2 eager + 1 delayed`、更保守的 delayed-only 或不纳入官方之前，不创建上游代码 PR；自用 fork 当前的强进策略保持不变。
...
| Hedged requests    | 自用 fork 继续强制开启当前策略。官方方向已建 Discussion #1953，只考虑默认关闭、有费用/隐私说明的独立设计；在社区范围定稿前不提代码 PR。                                                                          |
...
| 2026-08-23 | `0e50367` | `1881d5a`      | 发布 opt-in hedging Discussion #1953；自用行为未改，等待社区范围后再决定代码 PR |
