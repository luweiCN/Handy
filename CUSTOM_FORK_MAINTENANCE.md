...
| Hedged requests    | 自用 fork 继续强制开启当前策略。官方 Discussion #1953 已被维护者明确拒绝，不创建代码 PR；只有维护者未来主动改变方向或要求实现时才重新评估。                                                                      |
...
当前结论：`rejected by maintainer`。不提交对应代码 PR，也不通过改名、拆分或直接开 Draft PR 绕过该结论。后续同步 `upstream/main` 或调整本 fork 的 hedging 策略时，应复查该 Discussion；普通社区评论和票数可以补充记录，但只有维护者明确推翻原结论或主动要求实现，才重新开启上游贡献工作。

| 检查时间                   | Upvotes | 评论 | 结论与下一步                                                                                                                           |
| -------------------------- | ------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-23 19:38（+08:00） | 1       | 1    | `cjpais` 明确拒绝合入，理由为成本、UI/认知复杂度及项目转向本地模型。保留 fork-only 实现，不创建代码 PR；继续记录后续是否出现方向反转。 |
| 2026-08-23 19:41（+08:00） | 1       | 1    | 无新增评论、回复或 reaction；Discussion 仍 open 且未锁定，维护者结论未变。继续保持 fork-only，不创建代码 PR。                          |
| 2026-08-23 19:43（+08:00） | 1       | 1    | 应用户查看请求即时复查：评论者仍只有维护者 `cjpais`，楼中回复 0；维护者结论未变，不创建代码 PR。                                       |
| 2026-08-23 19:45（+08:00） | 1       | 1    | 无新增评论、回复或 reaction；Discussion 的 `updatedAt` 仍为维护者原评论时间。继续保持 fork-only，不创建代码 PR。                       |
